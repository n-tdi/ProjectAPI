...
import world.ntdi.projectapi.model.Base;
...
public class ProjectController {
    ...
    public ResponseEntity<Base> getProject(@PathVariable int id) {
        Project project = ProjectManager.getProjectFromId(id);
        if (project == null) return ResponseEntity.notFound().build();

        return ResponseEntity.ok(new Base(project.getTitle(), project.getDescription(), project.getSections(), project.getFeatures()));
    }
